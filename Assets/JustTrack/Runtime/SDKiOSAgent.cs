...
namespace JustTrack {
    internal class SDKiOSAgent : SDKBaseAgent {
        ...
        public override void NotifyLoadStart() {
            JustTrackSDKNativeBridgeUnity.NotifyLoadStart();
        }

        public override void NotifyLoadDone() {
            JustTrackSDKNativeBridgeUnity.NotifyLoadDone();
        }
        ...
        protected override void PerformInit(string pApiKey, string pTrackingId, string pTrackingProvider, string pCustomUserId, bool pAutomaticInAppPurchaseTracking, bool pIntegrateWithFirebase, bool pEnableDebugMode, Action<AttributionResponse> pOnSuccess, Action<string> pOnFailure) {
            ...
            userId = JustTrackSDKNativeBridgeUnity.Instance.Init(pApiKey, pTrackingId, pTrackingProvider, pCustomUserId, pAutomaticInAppPurchaseTracking, onSuccess, pOnFailure);
            if (pIntegrateWithFirebase) {
                JustTrackSDKNativeBridgeUnity.Instance.IntegrateWithFirebase();
            }
        }
        ...
        protected override void PerformAdColonyIntegration() {
            JustTrackSDKNativeBridgeUnity.Instance.IntegrateWithAdColony();
        }

        protected override void PerformAppLovinIntegration() {
            JustTrackSDKNativeBridgeUnity.Instance.IntegrateWithAppLovin();
        }

        protected override void PerformChartboostIntegration() {
            JustTrackSDKNativeBridgeUnity.Instance.IntegrateWithChartboost();
        }

        protected override void PerformUnityAdsIntegration() {
            JustTrackSDKNativeBridgeUnity.Instance.IntegrateWithUnityAds();
        }
        ...
        protected override bool PerformForwardAdImpression(string pAdFormat, string pAdSdkName, string pAdNetwork, string pPlacement, string pAbTesting, string pSegmentName, string pInstanceName, string pBundleId, Money pRevenue) {
            var value = pRevenue != null ? pRevenue.Value : 0.0;
            var currency = pRevenue != null ? pRevenue.Currency : "USD";

            return JustTrackSDKNativeBridgeUnity.Instance.ForwardAdImpression(pAdFormat, pAdSdkName, pAdNetwork, pPlacement, pAbTesting, pSegmentName, pInstanceName, pBundleId, value, currency);
        }
        ...
        protected override void PerformCreateAffiliateLink(string pChannel, Action<string> pOnSuccess, Action<string> pOnFailure) {
            JustTrackSDKNativeBridgeUnity.Instance.CreateAffiliateLink(pChannel, pOnSuccess, pOnFailure);
        }
        ...
        protected override void PerformGetAdvertiserIdInfo(Action<AdvertiserIdInfo> pOnSuccess, Action<string> pOnFailure) {
            string info = JustTrackSDKNativeBridgeUnity.Instance.GetAdvertiserIdInfo();
            JustTrackSDKBehaviour.CallOnMainThread(() => {
                pOnSuccess(AdvertiserIdInfo.CreateAdvertiserIdInfo(String.IsNullOrEmpty(info) ? null : info, String.IsNullOrEmpty(info)));
            });
        }
