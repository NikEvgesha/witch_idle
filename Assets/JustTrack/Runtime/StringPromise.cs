...
namespace JustTrack {
    internal class StringPromise : AndroidJavaProxy {
        private const string CLASS = "Promise";

        internal StringPromise(Action<string> pResolve, Action<string> pReject) : base($"{SDKAndroidAgent.PACKAGE}.{CLASS}") {
            m_OnResolve = pResolve;
            m_OnFailure = pReject;
        }

        Action<string> m_OnResolve;
